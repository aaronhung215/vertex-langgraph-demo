...
| **Cumulative LLM spend** | ~USD 0.02 for Blocks 1–4 + 5a runs. First eval pass cost USD ~13 (Ragas judge had thinking-on by default — see `eval/README.md` Cost post-mortem); judge switched to `gemini-2.5-flash-lite`, post-fix pass predicted ~USD 0.50, untested |
...
# UNTESTED at the time of writing. Verify with a 3-question sample first.
# (First-pass run cost USD ~13 — see eval/README.md Cost post-mortem.)
...
| **Demo total** | **~USD 0.02** |
| Eval pass (first run, judge thinking-on bug) | **USD ~13** — see `eval/README.md` Cost post-mortem |
| Eval pass (post-fix, judge = `gemini-2.5-flash-lite`) | ~USD 0.50 predicted — **untested**, verify with 3-question sample first |
